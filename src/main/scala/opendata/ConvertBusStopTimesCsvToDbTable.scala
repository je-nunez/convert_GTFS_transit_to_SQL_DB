...
class ConvertBusStopTimesCsvToDbTable(
  ...
  protected def parseAndValidateCsvLine(dataLine: String): Any = {
    ...
    val arrivalTime = lineValues(fieldsToCols("departure_time"))
    val departureTime = lineValues(fieldsToCols("departure_time"))
    val stopId = lineValues(fieldsToCols("stop_id"))
    val stopSequenceNumb = lineValues(fieldsToCols("stop_sequence")).toInt
    val pickupType = lineValues(fieldsToCols("pickup_type"))
    val dropOffType = lineValues(fieldsToCols("drop_off_type"))
  
    logMsg(DEBUG, s"Valid bus stop time: $tripId, $stopId at $arrivalTime")
    return DbSchemaStopTime(tripId, arrivalTime, departureTime, stopId,
                            stopSequenceNumb, pickupType, dropOffType)
