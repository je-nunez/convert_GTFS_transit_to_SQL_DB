...
class ConvertRouteShapesCsvToDbTable(
  ...
  protected def parseAndValidateCsvLine(dataLine: String): Any = {
    ...
    val shapePtSequencePos = lineValues(fieldsToCols("shape_pt_sequence")).toInt
    val shapePtLatitude = lineValues(fieldsToCols("shape_pt_lat")).toDouble
    val shapePtLongitude = lineValues(fieldsToCols("shape_pt_lon")).toDouble

    logMsg(DEBUG, s"Valid shape: $shapeId, $shapePtSequencePos ...")
    return DbSchemaRouteShape(shapeId, shapePtSequencePos, shapePtLatitude,
                              shapePtLongitude)
